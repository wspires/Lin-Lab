...
Matrix_Region::to_string( ) const
{
	...
	char buf[ BUFSIZ ];
	sprintf( buf, "[%u,%u]->[%u,%u]",
		static_cast<unsigned int>(row_begin()), static_cast<unsigned int>(col_begin()),
		static_cast<unsigned int>(row_begin() + row_size() - 1),
		static_cast<unsigned int>(col_begin() + col_size() - 1));
